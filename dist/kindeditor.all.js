...
function _inString(val, str, delimiter) {
    delimiter = delimiter === undefined$1 ? ',' : delimiter;
...
function _undef(val, defaultVal) {
    return val === undefined$1 ? defaultVal : val;
}
...
_extend(KEvent, {
    init: function (el, event) {
        ...
        if (!self.which && self.button !== undefined$1) {
...
function _bind(el, type, fn) {
    ...
    if (events.length === 0) {
        events[0] = function (e) {
            var kevent = e ? new KEvent(el, e) : undefined$1;
...
function _mediaImg(attrs) {
    ...
    var html = '<img class="' + _mediaClass(type) + '" src="' + K.EPNG + '" ' + (style !== '' ? 'style="' + style + '" ' : '') + 'data-ke-tag="' + escape(srcTag) + '" />';
    return html;
}
...
_extend(KNode, {
    ...
    opacity: function (val) {
        this.each(function () {
            if (this.style.opacity === undefined$1) {
    ...
    scan: function (fn, order) {
        ...
        order = order === undefined$1 ? true : order;
        function walk(node) {
...
_extend(KRange, {
    ...
    isControl: function () {
        var self = this, sc = self.startContainer, so = self.startOffset, ec = self.endContainer, eo = self.endOffset;
...
_extend(KCmd, {
    ...
    select: function (hasDummy) {
        ...
        if (_IERANGE) {
            ...
            sel.addRange(rng);
            if (doc !== document) {
                var selElm = K(rng.endContainer), pos = selElm.pos(), selH = selElm.height() + 10, scrollH = win.scrollY || win.pageYOffset, hideH = pos.y + selH - scrollH - win.innerHeight;
                if (hideH <= 0) {
                    return;
                }
...
var _IEJUSTIFY_TAG = 'justifyleft,justifycenter,justifyright,justifyfull'.split(','), _IELIST_TAG = 'formatblock,selectall,insertorderedlist,insertunorderedlist'.split(','), _IERANGE_TAG = _IEJUSTIFY_TAG.concat(_IELIST_TAG).concat('indent,outdent,subscript,superscript'.split(','));
...
function _drag(options) {
    var moveEl = options.moveEl, moveFn = options.moveFn, clickEl = options.clickEl || moveEl, beforeDrag = options.beforeDrag, iframeFix = options.iframeFix === undefined$1 ? true : options.iframeFix;
    var docs = [document];
    if (iframeFix) {
...
_extend(KWidget, {
    init: function (options) {
        ...
        if (self.z && (self.x === undefined$1 || self.y === undefined$1)) {
    ...
    autoPos: function (width, height) {
        var x, y, self = this, w = _removeUnit(width) || 0, h = _removeUnit(height) || 0, scrollPos = _getScrollPos();
        if (self._alignEl) {
...
function _getDefaultEditorStyle(full) {
    return ("<style>html{margin:0;padding:0;" + (full ? 'background:#F8F8F8' : '') + "}body{background:#fff;margin:0;padding:20px}body.full{margin:15px auto;width:800px;padding:30px;box-shadow:0 0 3px 2px rgba(0, 0, 0, 0.08)}body,td{font:1em/1.5 \"sans serif\",tahoma,verdana,helvetica;}body, p, div{word-wrap: break-word;}noscript {display:none;}table{border-collapse:collapse;}p{margin-bottom:2em}img::selection{background:rgba(0,0,0,.1)}img{font-size:0;border:0;vertical-align:bottom;max-width:100%;*max-width:99.9%;}img.ke-flash, img.ke-rm, img.ke-media{width:100%;height:460px;background-color:#dedede;background-repeat:no-repeat;background-position:center;background-image:url(\"data:image/svg+xml,%3Csvg viewBox='0 0 1024 1024' xmlns='http://www.w3.org/2000/svg' width='80' height='80'%3E%3Cpath d='M447.1 200L295.9 22.1H124.6L275.8 200h171.3zm444.8 0L740.7 22.1H569.4L720.7 200h171.2zm-222.4 0L518.3 22.1H347L498.2 200h171.3zM956.4 22.1H791.8L943.1 200h80.1V88.8c-.1-37.8-29-66.7-66.8-66.7zm-883 0h-6.7C29 22.1 0 51 0 88.8V200h224.6L73.4 22.1zM0 934c0 37.8 28.9 66.7 66.7 66.7h889.7c37.8 0 66.7-28.9 66.7-66.7V244.5H0V934zm355.9-489.3c0-37.8 31.1-55.6 66.7-55.6 11.1 0 24.5 2.2 35.6 8.9l253.5 146.8a63 63 0 0 1 0 111.2L458.2 802.8c-11.1 6.7-22.2 8.9-35.6 8.9-35.6 0-66.7-17.8-66.7-55.6V444.7zm0 0' fill='%23999999'/%3E%3C/svg%3E\");*width:99.9%;}img.ke-flash{background-image:url(\"data:image/svg+xml,%3Csvg viewBox='0 0 1024 1024' xmlns='http://www.w3.org/2000/svg' width='80' height='80'%3E%3Cpath d='M0 0v1024h1024V0H0zm766.427 327.534c-118.528-3.95-163.584 126.135-163.584 126.135H708.28v111.579s-64.146-.073-151.479 0c-93.038 233.216-250.66 234.46-292.864 233.143-.658-.037-8.667-.073-8.667-.073s.073-95.671-.074-111.8c62.062-.145 128.074-8.008 199.351-198.911 102.071-305.298 311.808-271.653 311.808-271.653v111.58z' fill='%23999999'/%3E%3C/svg%3E\")}img.ke-anchor {width:18px;height:18px;margin:0 3px;vertical-align:text-bottom;background:#70B4F1 url(\"data:image/svg+xml,%3Csvg viewBox='0 0 1024 1024' xmlns='http://www.w3.org/2000/svg' width='14' height='14'%3E%3Cpath d='M565.862 893.594a367.77 367.77 0 0 0 313.856-363.828 51.2 51.2 0 1 1 102.4 0 470.118 470.118 0 1 1-940.236 0 51.2 51.2 0 0 1 102.4 0 367.82 367.82 0 0 0 312.064 363.52V411.802A200.858 200.858 0 0 1 511.13 17.766a200.806 200.806 0 0 1 54.732 394.036v481.792zM511.078 309.76a91.29 91.29 0 1 0 0-182.528 91.29 91.29 0 0 0 0 182.528z' fill='%23ffffff'/%3E%3C/svg%3E\") no-repeat center;*background:#70B4F1}.ke-script, .ke-noscript, .ke-display-none{display:none}.ke-pagebreak {border:1px dotted #AAA;font-size:0;height:2px;}pre.prettyprint{padding: 10px;background: #F5F2F0;overflow: auto;user-select: none;color: #9c5425;font-family: Consolas,Monaco,'Andale Mono','Ubuntu Mono',monospace;}pre.prettyprint::selection{box-shadow: 0 0 1px 1px #7db7ff;}body.preview embed{width:100%;height:460px;}</style>");
}
function _getInitHtml(cssEmpty, bodyClass, cssPath, cssData, isFull) {
    ...
    var dir = _direction === '' ? '' : ' dir="' + _direction + '"', styleBody = bodyClass.length ? ' class="' + bodyClass.join(' ') + '"' : '', styleHeader = [];
    if (!cssEmpty) {
    ...
    return ("<html" + dir + "><head><meta charset=\"utf-8\" /><title></title>" + styleHeader + "</head><body" + styleBody + "></body></html>");
}
...
_extend(KEdit, KWidget, {
    ...
    design: function (bool) {
        var self = this, val;
        if (bool === undefined$1 ? !self.designMode : bool) {
            if (!self.designMode) {
                val = self.html();
...
_extend(KToolbar, KWidget, {
    init: function (options) {
        ...
        self.div.addClass('ke-toolbar').bind('contextmenu,mousedown,mousemove', function (e) {
            e.preventDefault();
        }).attr('unselectable', 'on').css('display', '');
    ...
    disableAll: function (bool, noDisableItems) {
        ...
        if (bool === undefined$1 ? !self.disableMode : bool) {
...
_extend(KUploadButton, {
    init: function (options) {
        var self = this, useForm = options.form, button = K(options.button), fieldName = options.fieldName || 'file', url = options.url || '', title = button.val(), cls = button[0].className || '', target = 'kindeditor_upload_iframe_' + new Date().getTime();
        ...
        html.push('<input type="file" class="ke-upload-file" name="' + fieldName + '" tabindex="-1" />', useForm ? '</form>' : '</div>', '</div>');
        html = html.join('');
    ...
    formSubmit: function () {
        var self = this, iframe = self.iframe, form = self.form[0];
    ...
    submit: function () {
        ...
        var xhr = new XMLHttpRequest(), formData = new FormData(), extraParams = _makeExtraParams(options.extraParams);
        ...
        xhr.onload = function () {
            ...
            if (xhr.status !== 200) {
                data = {
                    error: 1,
                    message: 'Response Http:' + xhr.status
                };
            } else {
                try {
...
function _createButton(arg) {
    arg = arg || {};
    var name = arg.name || '', span = K('<span class="ke-button-outer" title="' + name + '"></span>'), btn = K('<input class="ke-button" type="button" value="' + name + '" />');
    if (arg.click) {
        btn.click(arg.click);
    }
    span.append(btn);
...
_extend(KDialog, KWidget, {
    ...
    showLoading: function (msg) {
        msg = _undef(msg, '');
        var self = this, body = self.bodyDiv, svgLoad = '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" style="display:block;margin:auto;width:90px;height:90px" viewBox="0 0 100 100" preserveAspectRatio="xMidYMid">', headHeight = self.headerDiv.height(), loadTop = headHeight + 55;
        for (var i = 0; i < 9; i++) {
            svgLoad += "<g transform=\"rotate(" + (45 * i) + " 50 50)\"><rect x=\"47\" y=\"24\" rx=\"3\" ry=\"6\" width=\"6\" height=\"12\" fill=\"#bbbbbb\"><animate attributeName=\"opacity\" values=\"1;0\" keyTimes=\"0;1\" dur=\"1s\" begin=\"" + (-0.125 * (8 - i)) + "s\" repeatCount=\"indefinite\"></animate></rect></g>";
        }
        svgLoad += '</svg>';
        self.loading = K('<div class="ke-dialog-loading"><div class="ke-inline-block ke-dialog-loading-content" style="margin-top:' + Math.round(body.height() / 2 - loadTop) + 'px;">' + svgLoad + msg + '</div></div>').width(body.width()).height(body.height()).css('top', headHeight + 'px');
...
function _tabs(options) {
    ...
    div.addClass(options.addClass ? options.addClass : 'ke-tabs').bind('contextmenu,mousedown,mousemove', function (e) {
        e.preventDefault();
    });
    ...
    self.select = function (index) {
        ...
        _each(liList, function (i, li) {
            ...
            if (i === index) {
                ...
                li.removeClass('selected').removeClass('on').mouseover(function () {
                    K(this).addClass('on');
                }).mouseout(function () {
...
function _lang(mixed, langType) {
    langType = langType === undefined$1 ? K.options.langType : langType;
...
function KEditor(options) {
    ...
    function setOption(key, val) {
        if (KEditor.prototype[key] === undefined$1) {
            self[key] = val;
        }
...
KEditor.prototype = {
    ...
    loadPlugin: function (name, fn) {
        ...
        var alias = self.pluginAlias || {}, pathName = name;
        for (var k in alias) {
    ...
    create: function () {
        ...
        container.removeClass('container').addClass('ke-container ke-container-' + self.themeType + (fullscreenMode ? ' ke-container-full' : '')).css('width', width);
        if (fullscreenMode) {
            container.css({
                position: 'absolute',
                left: 0,
    ...
    fullHtml: function () {
        return this.isCreated ? this.edit.html(undefined$1, true) : '';
    },
    text: function (val) {
        var self = this;
        if (val === undefined$1) {
    ...
    fullscreen: function (bool) {
        this.fullscreenMode = bool === undefined$1 ? !this.fullscreenMode : bool;
    ...
    createMenu: function (options) {
        ...
        if (options.selectedColor !== undefined$1) {
...
_plugin('core', function (K) {
    ...
    _each('anchor,link,image,flash,media'.split(','), function (i, name) {
        ...
        _each('edit,remove'.split(','), function (j, val) {
            self.addContextmenu({
                ...
                width: 150,
                iconClass: val == 'edit' ? 'ke-icon-' + name : undefined$1
            });
        });
    ...
    _each(('prop,rowprop,cellprop,colinsertleft,colinsertright,rowinsertabove,rowinsertbelow,rowmerge,colmerge,' + 'rowsplit,colsplit,coldelete,rowdelete,insert,remove').split(','), function (i, val) {
...
KindEditor.lang({
    ...
    'code.edit': '修改代码块',
    'code.remove': '删除代码块',
    'quickformat.center': '图片居中',
    'quickformat.centerIndent': '图片居中 + 段落缩进',
    'quickformat.left': '图片居左',
    'quickformat.leftIndent': '图片居左 + 段落缩进',
    'quickformat.unlink': '仅去除超链接',
    'link.url': 'URL',
    'link.linkType': '打开类型',
    'link.newWindow': '新窗口',
    ...
    'table.padding': '边距',
    'table.spacing': '间距',
    'table.borderWidth': '边框宽度',
    'table.borderColor': '边框颜色',
    'table.align': '对齐方式',
...
KindEditor.plugin('anchor', function (K) {
    ...
    self.plugin.anchor = {
        edit: function () {
            var html = "<div style=\"padding:20px;\"><div class=\"ke-dialog-row\"><label for=\"keName\">" + (lang.name) + "</label><input type=\"text\" id=\"keName\" name=\"name\" value=\"\" style=\"width:170px;\" /></div></div>";
            var dialog = self.createDialog({
                name: name,
                width: 300,
...
KindEditor.plugin('baidumap', function (K) {
    var self = this, name = 'baidumap', lang = self.lang(name + '.'), mapWidth = K.undef(self.mapWidth, 558), mapHeight = K.undef(self.mapHeight, 360), mapHtml = K.undef(self.mapHtml, self.pluginsPath + 'baidumap/index.html');
    function getMapHtml() {
        return "<html><head><meta charset=\"utf-8\" /><title></title><style>html,body {width:100%;height:100%;margin:0;padding:0;background:#FFF }</style><link rel=\"stylesheet\" type=\"text/css\" href=\"http://api.map.baidu.com/res/13/bmap.css\"/><script type=\"text/javascript\" src=\"http://api.map.baidu.com/getscript?v=1.3&ak=&services=&t=20200103072755\"></script><script>var map,geocoder;function initialize(){map=new BMap.Map(\"map_canvas\");var a=new BMap.Point(121.473704,31.230393);map.centerAndZoom(a,11),map.addControl(new BMap.NavigationControl),map.enableScrollWheelZoom(),(new BMap.Geocoder).getLocation(a,function(a){var e=[a.addressComponents.city].join(\"\");parent.document.getElementById(\"kindeditor_plugin_map_address\").value=e})}function search(a){map&&new BMap.LocalSearch(map,{renderOptions:{map:map,autoViewport:!0,selectFirstResult:!1}}).search(a)}</script></head><body onload=\"initialize();\"><div id=\"map_canvas\" style=\"width:100%; height:100%\"></div></body></html>";
    }
    self.clickToolbar(name, function () {
        var html = "<div style=\"padding:10px 20px;\"><div class=\"ke-header\"><div class=\"ke-left\">" + (lang.address) + " <input type=\"text\" id=\"kindeditor_plugin_map_address\" name=\"address\" value=\"\" style=\"width:200px;margin-right:4px\" /><span class=\"ke-button-outer\"><button name=\"searchBtn\" class=\"ke-button\">" + (lang.search) + "</button></span></div><div class=\"ke-right\" style=\"margin-top:6px\"><input type=\"checkbox\" id=\"keInsertDynamicMap\" name=\"insertDynamicMap\" value=\"1\" /> <label for=\"keInsertDynamicMap\">" + (lang.insertDynamicMap) + "</label></div><div class=\"ke-clearfix\"></div></div><div class=\"ke-map\" style=\"width:" + mapWidth + "px;height:" + mapHeight + "px;\"></div></div>";
        var dialog = self.createDialog({
            name: name,
            width: mapWidth + 42,
            title: self.lang(name),
            ...
            yesBtn: {
                ...
                click: function (e) {
                    ...
                    var url = [
                        checkbox[0].checked ? mapHtml : 'http://api.map.baidu.com/staticimage',
        ...
        var div = dialog.div, addressBox = K('[name="address"]', div), searchBtn = K('[name="searchBtn"]', div), checkbox = K('[name="insertDynamicMap"]', dialog.div), srcScript = 'document.open();document.close();', iframeSrc = K._IE ? ' src="javascript:void(function(){' + encodeURIComponent(srcScript) + '}())"' : '', iframe = K('<iframe class="ke-textarea" frameborder="0" style="width:100%;height:100%"' + iframeSrc + '></iframe>'), win, doc;
        function ready() {
...
KindEditor.plugin('code', function (K) {
    var self = this, name = 'code', title = self.lang(name), lang = self.lang(name + '.'), codes = {
            js: 'JavaScript',
            html: 'HTML',
            css: 'CSS',
            php: 'PHP',
            ...
            bsh: 'Shell',
            code: 'Other'
        }, html = '<div style="padding:10px 20px;"><div class="ke-dialog-row"><select class="ke-code-type">', hackIe = K.IE && K.V < 9, contextBlock;
    ...
    function openDialog(elm) {
        var dialog = self.createDialog({
                ...
                yesBtn: {
                    name: self.lang('yes'),
                    click: function (e) {
                        var type = K('.ke-code-type', dialog.div).val(), code = textarea.val(), cls = 'prettyprint' + (type === '' ? '' : ' lang-' + type);
                        if (K.trim(code) === '') {
...
KindEditor.plugin('emoticons', function (K) {
    ...
    var emojiGroup = [
        {
            name: 'QQ表情',
            preview: self.allowPreviewEmoticons === undefined$1 ? true : self.allowPreviewEmoticons,
            baseUrl: qqPath,
            rows: 10,
            lists: [[
                    0,
            ...
            insertCell: function (src, index) {
                return '<span class="ke-qqimg" style="background-image:url(' + qqPath + 'static.png);' + 'background-position:-' + 24 * index + 'px 0px' + '"></span>';
            },
        ...
        for (i = 0; i < len; i++) {
            if (K.isArray(group.lists[i])) {
                ...
                for (j = group.lists[i][0]; j <= group.lists[i][1]; j++) {
                    emojis.push((size > 3 ? group.lists[i][3] : '') + '' + j + '' + (size > 2 ? group.lists[i][2] : ''));
                }
            } else {
            ...
            for (j = 0; j < group.rows; j++) {
                ...
                if (size < len) {
                    ...
                    cell.attr({
                        src: src,
                        index: size
                    }).append(group.insertCell ? group.insertCell(src, size) : '<img src="' + src + '" />');
                    elements.push(cell);
...
KindEditor.plugin('filemanager', function (K) {
    var self = this, name = 'filemanager', fileManagerJson = K.undef(self.fileManagerJson, self.basePath + 'php/file_manager_json.php'), lang = self.lang(name + '.');
    ...
    self.plugin.filemanagerDialog = function (options) {
        var width = K.undef(options.width, 650), dirName = K.undef(options.dirName, ''), viewType = K.undef(options.viewType, 'VIEW').toUpperCase(), clickFn = options.clickFn, html = "<div class=\"ke-plugin-filemanager\"><div class=\"ke-header ke-plugin-filemanager-header\"><div class=\"ke-left\"><a class=\"ke-inline-block\" name=\"moveupLink\" href=\"javascript:;\">" + (lang.moveup) + "</a></div><div class=\"ke-right\"> " + (lang.viewType) + " <select class=\"ke-inline-block\" name=\"viewType\"><option value=\"VIEW\">" + (lang.viewImage) + "</option><option value=\"LIST\">" + (lang.listImage) + "</option></select>\t " + (lang.orderType) + "<select class=\"ke-inline-block\" name=\"orderType\"><option value=\"NAME\">" + (lang.fileName) + "</option><option value=\"SIZE\">" + (lang.fileSize) + "</option><option value=\"TYPE\">" + (lang.fileType) + "</option></select></div><div class=\"ke-clearfix\"></div></div><div class=\"ke-plugin-filemanager-body\"></div></div>";
        var dialog = self.createDialog({
                name: name,
                width: width,
                title: self.lang(name),
                body: html
            }), div = dialog.div, moveupLink = K('[name="moveupLink"]', div), viewTypeBox = K('[name="viewType"]', div), orderTypeBox = K('[name="orderType"]', div), bodyDiv = K('.ke-plugin-filemanager-body', div);
        ...
        function createList(result) {
            ...
            for (var i = 0, len = fileList.length; i < len; i++) {
                ...
                var img = getSvg(data.is_dir, false), cell0 = K(row[0].insertCell(0)).addClass('ke-cell ke-name').append(img).append(document.createTextNode(' ' + data.filename)).attr('title', data.filename);
        ...
        function createView(result) {
            ...
            var fileList = result.file_list;
            for (var i = 0, len = fileList.length; i < len; i++) {
                var data = fileList[i], div = K('<div class="ke-inline-block ke-item"></div>'), photoDiv = K('<div class="ke-inline-block ke-photo"></div>').mouseover(function (e) {
                        K(this).addClass('ke-on');
                    }).mouseout(function (e) {
                        K(this).removeClass('ke-on');
                    });
                ...
                div.append(photoDiv.append(getFilePreview(result, data))).append('<div class="ke-name" title="' + data.filename + '">' + data.filename + '</div>').appendTo(bodyDiv);
            }
        }
...
var _file_useForm, _file_uploadJson, _file_name, _file_params, _file_formatUrl, _file_fillDescAfter, _file_allowManager, _file_uploadDirs = {
        insertfile: 'file',
        flash: 'flash',
        media: 'media',
        image: 'image'
...
function openFileDialog(self, K, name, fileUrl, fileTitle, allowUpload, options) {
    var lang = self.lang(name + '.'), isMedia = name === 'media', isImage = !isMedia && name === 'image', clickFn = options ? options.clickFn : null;
    html = "<div style=\"padding:20px;\"><div class=\"ke-dialog-row\"><label for=\"keUrl\">" + (lang.url) + "</label><input type=\"text\" id=\"keUrl\" name=\"url\" style=\"width:205px;\" /><input type=\"button\" class=\"ke-upload-button\" value=\"" + (lang.upload) + "\" /><span class=\"ke-button-outer\" style=\"margin-left:5px;\"><input type=\"button\" class=\"ke-button\" name=\"viewServer\" value=\"" + (lang.viewServer) + "\" /></span></div>";
    if (name === 'insertfile') {
        html += "<div class=\"ke-dialog-row\"><label for=\"keTitle\">" + (lang.title) + "</label><input type=\"text\" id=\"keTitle\" name=\"title\" style=\"width:335px;\" /></div></div>";
    } else if (isMedia || name === 'flash') {
        html += "<div class=\"ke-dialog-row\"><label for=\"keWidth\">" + (lang.width) + "</label><input type=\"text\" id=\"keWidth\" class=\"ke-input-number\" name=\"width\" maxlength=\"4\" /> &nbsp;" + (lang.height) + " &nbsp; <input type=\"text\" id=\"keHeight\" class=\"ke-input-number\" name=\"height\" maxlength=\"4\" /> &nbsp;" + (isMedia ? '<input type="checkbox" id="keAutostart" name="autostart" style="vertical-align:middle"/><label for="keAutostart" style="width:auto">' + lang.autostart + '</label>' : '') + "</div>";
    } else {
        html += "<div class=\"ke-dialog-row\"><label>" + (lang.align) + "</label><label title=\"" + (lang.defaultAlign) + "\"><input type=\"radio\" name=\"align\" class=\"ke-inline-block\" value=\"\" checked=\"checked\" />" + (getAlignSvg(0)) + "</label><label title=\"" + (lang.leftAlign) + "\"><input type=\"radio\" name=\"align\" class=\"ke-inline-block\" value=\"left\" />" + (getAlignSvg(1)) + "</label><label title=\"" + (lang.rightAlign) + "\"><input type=\"radio\" name=\"align\" class=\"ke-inline-block\" value=\"right\" />" + (getAlignSvg(2)) + "</label></div><div class=\"ke-dialog-row\"><label for=\"remoteTitle\">" + (lang.imgTitle) + "</label><input type=\"text\" id=\"remoteTitle\" name=\"title\" style=\"width:200px;\" /></div><div class=\"ke-dialog-row\"><label for=\"remoteWidth\">" + (lang.size) + "</label>" + (lang.width) + "&nbsp;<input type=\"text\" id=\"remoteWidth\" class=\"ke-input-number\" name=\"width\" maxlength=\"4\" />&nbsp;" + (lang.height) + "&nbsp;<input type=\"text\" class=\"ke-input-number\" name=\"height\" maxlength=\"4\" />&nbsp;<span title=\"" + (lang.keepAspect) + "\" class=\"ke-img-svg ke-img-svgon\"><svg viewBox=\"0 0 1024 1024\" xmlns=\"http://www.w3.org/2000/svg\" width=\"18\" height=\"18\"><path d=\"M839.6 430.207h-72.59V261.461c0-141.567-114.732-256.3-256.3-256.3s-256.301 114.733-256.301 256.3v168.746H181.99c-34.919 0-63.301 28.383-63.301 63.301V955.71c0 16.858 6.708 32.855 18.577 44.896 11.87 11.87 28.039 18.578 44.896 18.578h658.47c16.857 0 32.855-6.709 44.896-18.578 11.869-11.869 18.577-28.038 18.577-44.896V493.508c0-16.857-6.708-33.198-18.75-45.067-12.556-11.87-28.898-18.406-45.755-18.234zm-170.981 0H352.8v-164.79c0-87.21 70.698-157.908 157.909-157.908s157.909 70.698 157.909 157.909v164.79zm0 0\"/></svg></span><span title=\"" + (lang.resetSize) + "\" class=\"ke-img-svg\"><svg viewBox=\"0 0 1024 1024\" xmlns=\"http://www.w3.org/2000/svg\" width=\"18\" height=\"18\"><path d=\"M232.4 362.7c14.7-27.3 33.2-52.2 55.4-74.4C317.1 259 351.2 236 389.1 220c39.2-16.6 80.9-25 123.9-25 39.3 0 77.5 7 113.8 20.9 35 13.4 67.2 32.8 95.6 57.6 52.8 46.1 88.8 108.1 102.7 176H955v-.2C923.8 233.1 737.8 67 513 67c-156.3 0-293.9 80.3-373.6 202L65 194v256h254l-86.6-87.3zM960 577H706l87.2 87.9c-18 33.2-41.9 63.3-70.9 88.6-28.4 24.8-60.6 44.2-95.6 57.6C590.5 825 552.3 832 513 832s-77.5-7-113.7-20.9c-35-13.4-67.2-32.8-95.6-57.6-52.8-46.1-88.9-108.1-102.7-176.1H71.1c0 .1 0 .3.1.4C102.3 794 288.3 960 513 960c156.1 0 293.4-80.1 373.2-201.3L960 833V577z\"/></svg></span><a style=\"color:#70B4F1;cursor:pointer\">clear</a></div>";
    }
    html += '</div>';
    var titleBox, widthBox, heightBox, alignBox, locksizeBtn, refreshBtn, clearSizeBtn, autostartBox, dialog = self.createDialog({
            name: name,
            width: 500,
            title: self.lang(name),
            body: html,
            yesBtn: {
                ...
                click: function (e) {
                    ...
                    if (name === 'insertfile') {
                        ...
                        var width = widthBox.val(), height = heightBox.val();
                        if (isImage) {
        ...
        }), div = dialog.div, urlBox = K('[name="url"]', div), viewServerBtn = K('[name="viewServer"]', div), dirName = _file_uploadDirs[name], addWidth = 0;
    if (allowUpload) {
    ...
    if (name === 'insertfile') {
        ...
        titleBox = K('[name="title"]', div).val(fileTitle);
        var lockAspect = true, originalWidth = 0, originalHeight = 0, setSize = function (width, height) {
                widthBox.val(width);
        ...
        setSize(K.undef(options.imageWidth, ''), K.undef(options.imageHeight, ''));
...
(function () {
    ...
    for (i = 0; i < _file_plugins.length; i++) {
        (function (name) {
            KindEditor.plugin(name, function (K) {
                var self = this, allowUpload = K.undef(self[configNames[name]], true);
                if (!_file_uploadJson) {
                    _file_useForm = K.undef(self.forceIframeTransport, false) || window.FormData === undefined$1;
                ...
                if (name === 'insertfile') {
                    ...
                    self.plugin.imageDialog = function (options) {
                        var fileUrl = K.undef(options.imageUrl, 'http://'), fileTitle = K.undef(options.imageTitle, ''), showLocal = allowUpload;
        ...
        K.each(lang.lineHeight, function (i, row) {
            K.each(row, function (key, val) {
                menu.addItem({
                    ...
                    checked: curVal === key,
                    click: function () {
                        self.cmd.toggle('<span style="line-height:' + key + ';"></span>', { span: '.line-height=' + key });
                        self.updateState();
                        self.addBookmark();
                        self.hideMenu();
                    }
...
KindEditor.plugin('link', function (K) {
    ...
    self.plugin.link = {
        edit: function () {
            var lang = self.lang(name + '.'), html = "<div style=\"padding:20px;\"><div class=\"ke-dialog-row\"><label for=\"keUrl\" style=\"width:60px;\">" + (lang.url) + "</label><input type=\"text\" id=\"keUrl\" name=\"url\" style=\"width:306px;\" /></div><div class=\"ke-dialog-row\"\"><label for=\"keType\" style=\"width:60px;\">" + (lang.linkType) + "</label><select id=\"keType\" name=\"type\"></select></div></div>", dialog = self.createDialog({
                    name: name,
                    width: 450,
                    title: self.lang(name),
                    body: html,
...
KindEditor.plugin('plainpaste', function (K) {
    ...
    self.clickToolbar(name, function () {
        var lang = self.lang(name + '.'), html = "<div style=\"padding:10px 20px;\"><div style=\"margin-bottom:10px;\">" + (lang.comment) + "</div><textarea class=\"ke-textarea\" style=\"width:100%;height:340px;\"></textarea></div>", dialog = self.createDialog({
                name: name,
                width: 560,
                title: self.lang(name),
                body: html,
...
KindEditor.plugin('preview', function (K) {
    ...
    self.clickToolbar(name, function () {
        var html = "<div style=\"border-top:1px solid #dadada;\"><iframe class=\"ke-preview\" frameborder=\"0\" style=\"width:100%;height:500px;\"></iframe></div>", dialog = self.createDialog({
                name: name,
                width: 780,
                title: self.lang(name),
                body: html
...
KindEditor.plugin('quickformat', function (K) {
    var self = this, name = 'quickformat', lang = self.lang(name + '.'), IE8 = K.IE && K.V < 9;
    ...
    function quickformat(indent, imgCenter) {
        var win = self.edit.win, doc = self.edit.doc, body = doc.body, placeHolder = '##___keQuick___##', placeLength = placeHolder.length;
        var preNodes = {};
        var replaceNode = function (tag, fn) {
            var nodes = doc.getElementsByTagName(tag), len = nodes.length;
            if (!len) {
                return;
            }
            if (!(tag in preNodes)) {
        ...
        var getReplaceNative = function (txt) {
            if (txt.substr(0, placeLength) === placeHolder) {
                var str = txt.substr(placeLength).split('_'), type = str[0], key = parseInt(str[1]);
                if (preNodes[type] && preNodes[type][key]) {
                    return preNodes[type][key];
                }
            }
        };
        var docRange = win.getSelection && typeof doc.createRange != 'undefined', bodyRange = !docRange && typeof doc.selection != 'undefined' && typeof body.createTextRange != 'undefined';
    ...
    function quickAction(type) {
        ...
        if (type === 'unlink') {
            ...
            var center = type.indexOf('center') > -1, indent = type.indexOf('Indent') > -1;
...
KindEditor.plugin('table', function (K) {
    ...
    self.plugin.table = {
        quick: function () {
            var menu, html, i, table, selected, status, nums, more, row, col, wrapperDiv = K('<div class="ke-plugin-qtable"></div>');
            ...
            table.bind('mousemove', function (ev) {
                if (ev.target.tagName.toLowerCase() === 'li') {
                    var index = ev.target.getAttribute('index') * 1, newRow = Math.floor(index / 10) + 1, newCol = index % 10 + 1;
        ...
        prop: function (isInsert) {
            var html = "<div style=\"padding:20px;\"><div class=\"ke-dialog-row\"><label for=\"keRows\">" + (lang.rows) + "</label><input type=\"text\" id=\"keRows\" name=\"rows\" maxlength=\"4\" style=\"width:96px;\"/><label for=\"keCols\" style=\"margin-left:40px\">" + (lang.cols) + "</label><input type=\"text\" id=\"keCols\" name=\"cols\" maxlength=\"4\" style=\"width:96px;\"/></div><div class=\"ke-dialog-row\"><label for=\"keWidth\">" + (lang.width) + "</label><input type=\"text\" id=\"keWidth\" class=\"ke-input-number\" name=\"width\" maxlength=\"4\" /><select name=\"widthType\" style=\"border-left:0\"><option value=\"%\">" + (lang.percent) + "</option><option value=\"px\">" + (lang.px) + "</option></select><label for=\"keHeight\" style=\"margin-left:40px\">" + (lang.height) + "</label><input id=\"keHeight\" type=\"text\" class=\"ke-input-number\" name=\"height\" maxlength=\"4\" /><select name=\"heightType\" style=\"border-left:0\"><option value=\"%\">" + (lang.percent) + "</option><option value=\"px\">" + (lang.px) + "</option></select></div><div class=\"ke-dialog-row\"><label for=\"kePadding\">" + (lang.padding) + "</label><input type=\"text\" id=\"kePadding\" name=\"padding\" maxlength=\"4\" style=\"width:96px;\"/><label for=\"keSpacing\" style=\"margin-left:40px\">" + (lang.spacing) + "</label><input type=\"text\" id=\"keSpacing\" name=\"spacing\" maxlength=\"4\" style=\"width:96px;\" /></div><div class=\"ke-dialog-row\"><label for=\"keBorderWidth\">" + (lang.borderWidth) + "</label><input type=\"text\" id=\"keBorderWidth\" name=\"border\" maxlength=\"4\" style=\"width:96px;\"/><label style=\"margin-left:40px\">" + (lang.borderColor) + "</label><span class=\"ke-inline-block ke-input-color\" style=\"width:101px;\"></span></div><div class=\"ke-dialog-row\"><label>" + (lang.align) + "</label><select name=\"align\" style=\"width:106px;\"><option value=\"\">" + (lang.alignDefault) + "</option><option value=\"left\">" + (lang.alignLeft) + "</option><option value=\"center\">" + (lang.alignCenter) + "</option><option value=\"right\">" + (lang.alignRight) + "</option></select><label style=\"margin-left:40px\">" + (lang.backgroundColor) + "</label><span class=\"ke-inline-block ke-input-color\" style=\"width:101px;\"></span></div></div>";
                    ...
                    yesBtn: {
                        ...
                        click: function (e) {
                            ...
                            if (table) {
                                ...
                                if (table[0].height !== undefined$1) {
                                    table.removeAttr('height');
                                }
                                table.css('background-color', bgColor);
                                if (table[0].bgColor !== undefined$1) {
            ...
            if (table) {
                ...
                if (tableWidth !== undefined$1 && (match = /^(\d+)((?:px|%)*)$/.exec(tableWidth))) {
                    widthBox.val(match[1]);
                    widthTypeBox.val(match[2]);
                } else {
                    widthBox.val('');
                }
                if (tableHeight !== undefined$1 && (match = /^(\d+)((?:px|%)*)$/.exec(tableHeight))) {
                ...
                borderBox.val(table[0].border === undefined$1 ? '' : table[0].border);
        ...
        modifyProp: function (cell) {
            var html = "<div style=\"padding:20px;\">" + (cell ? ("<div class=\"ke-dialog-row\"><label for=\"keWidth\">" + (lang.width) + "</label><input type=\"text\" id=\"keWidth\" class=\"ke-input-number\" name=\"width\" maxlength=\"4\" /><select name=\"widthType\" style=\"border-left:0\"><option value=\"%\">" + (lang.percent) + "</option><option value=\"px\">" + (lang.px) + "</option></select><label for=\"keHeight\" style=\"margin-left:40px\">" + (lang.height) + "</label><input type=\"text\" id=\"keHeight\" class=\"ke-input-number\" name=\"height\" maxlength=\"4\" /><select name=\"heightType\" style=\"border-left:0\"><option value=\"%\">" + (lang.percent) + "</option><option value=\"px\">" + (lang.px) + "</option></select></div>") : ("<div class=\"ke-dialog-row\"><label for=\"keHeight\">" + (lang.height) + "</label><input type=\"text\" id=\"keHeight\" class=\"ke-input-number\" name=\"height\" maxlength=\"4\" style=\"width:101px\"/></div>")) + "<div class=\"ke-dialog-row\"><label for=\"keAlign\">" + (lang.textAlign) + "</label><select id=\"keAlign\" name=\"textAlign\" style=\"width:106px\"><option value=\"\">" + (lang.alignDefault) + "</option><option value=\"left\">" + (lang.alignLeft) + "</option><option value=\"center\">" + (lang.alignCenter) + "</option><option value=\"right\">" + (lang.alignRight) + "</option></select><label for=\"verticalAlign\" style=\"margin-left:40px\">" + (lang.verticalAlign) + "</label><select name=\"verticalAlign\" style=\"width:106px\"><option value=\"\">" + (lang.alignDefault) + "</option><option value=\"top\">" + (lang.alignTop) + "</option><option value=\"middle\">" + (lang.alignMiddle) + "</option><option value=\"bottom\">" + (lang.alignBottom) + "</option><option value=\"baseline\">" + (lang.alignBaseline) + "</option></select></div><div class=\"ke-dialog-row\"><label>" + (lang.borderColor) + "</label><span class=\"ke-inline-block ke-input-color\" style=\"width:101px\"></span><label style=\"margin-left:40px\">" + (lang.backgroundColor) + "</label><span class=\"ke-inline-block ke-input-color\" style=\"width:101px\"></span></div>" + (cell ? ("<div class=\"ke-dialog-row\"><label for=\"keBorder\">" + (lang.borderWidth) + "</label><input type=\"text\" id=\"keBorder\" class=\"ke-input-number\" name=\"border\" maxlength=\"4\" style=\"width:96px\"/></div>") : '') + "\t</div>";
            var bookmark = self.cmd.range.createBookmark();
            var dialog = self.createDialog({
                    name: name,
                    width: 470,
                    ...
                    yesBtn: {
                        name: self.lang('yes'),
                        click: function (e) {
                            var height = heightBox.val(), textAlign = textAlignBox.val(), verticalAlign = verticalAlignBox.val(), borderColor = K(colorBox[0]).html() || '', bgColor = K(colorBox[1]).html() || '';
                            if (cell) {
                                var width = widthBox.val(), widthType = widthTypeBox.val(), heightType = heightTypeBox.val(), border = borderBox.val();
                                if (!/^\d*$/.test(width)) {
                                    alert(self.lang('invalidWidth'));
                                    widthBox[0].focus();
                                    return;
                ...
                }), div = dialog.div, heightBox = K('[name="height"]', div), textAlignBox = K('[name="textAlign"]', div), verticalAlignBox = K('[name="verticalAlign"]', div), colorBox = K('.ke-input-color', div), heightTypeBox = cell ? K('[name="heightType"]', div) : null, widthBox = cell ? K('[name="width"]', div) : null, widthTypeBox = cell ? K('[name="widthType"]', div) : null, borderBox = cell ? K('[name="border"]', div) : null;
            var section = cell ? self.plugin.getSelectedCell() : self.plugin.getSelectedRow(), secelm = section[0], style = secelm.style;
            _initColorPicker(div, colorBox.eq(0));
            _initColorPicker(div, colorBox.eq(1));
...
KindEditor.plugin('template', function (K) {
    ...
    self.clickToolbar(name, function () {
        ...
        var html = "<div style=\"padding:10px 20px;\"><div class=\"ke-header\"><div class=\"ke-left\">" + (lang.selectTemplate) + " <select>" + options + "</select></div><div class=\"ke-right\"><input type=\"checkbox\" id=\"keReplaceFlag\" name=\"replaceFlag\" value=\"1\" /> <label for=\"keReplaceFlag\">" + (lang.replaceContent) + "</label></div><div class=\"ke-clearfix\"></div></div><iframe class=\"ke-textarea\" frameborder=\"0\" style=\"width:100%;height:340px;background-color:#FFF;\"></iframe></div>";
        var dialog = self.createDialog({
            name: name,
            width: 560,
            title: self.lang(name),
...
KindEditor.plugin('wordpaste', function (K) {
    ...
    self.clickToolbar(name, function () {
        var lang = self.lang(name + '.'), html = "<div style=\"padding:10px 20px;\"><div style=\"margin-bottom:10px;\">" + (lang.comment) + "</div><iframe class=\"ke-textarea\" frameborder=\"0\" style=\"width:100%;height:340px;\"></iframe></div>", dialog = self.createDialog({
                name: name,
                width: 560,
                title: self.lang(name),
                body: html,
